	...
	if(!empty($imagem["name"]))
	{
		...
		{ 
			...
			/*//--------------------------------------------------------------------
			//Verificando se a largura da imagem upada é maior que a permitida
			if($dimensoes[0] > $largura)
			{
				$error[1] = "A largura não deve ser maior que " . $largura;
				$cont++;
			}
			
			if($dimensoes[1] > $altura)
			{
				$error[2] = "A altura não deve ser maior que " . $altura;
				$cont++;
			}
			*///--------------------------------------------------------------------
			...
			if ($cont == 0)
			{
				...
				/*
				//--------------------------------------------------------------------
				//Pegando o título do catálogo
				$con = open_con();
				
				$sql = "SELECT nome FROM cs_catalogos WHERE id = '$id'";
				...
				*/
				...
				$caminho_imagem = $_SERVER['DOCUMENT_ROOT'] . "/Catalogo/edicao/galeria/" . $id . "/paginas/" . $nome_imagem;
				...
				$string_banco = $_SERVER['DOCUMENT_ROOT'] . "/Catalogo/edicao/galeria/" . $id . "/paginas/" . $nome_imagem;
				...
				$imagem = wideImage::load($_SERVER['DOCUMENT_ROOT'] . "/Catalogo/edicao/galeria/" . $id . "/paginas/" . $nome_imagem);

				//Redimensionando
				$imagem = $imagem->resize(400, 300);
				
				$caminho_thumb = $_SERVER['DOCUMENT_ROOT'] . "/Catalogo/edicao/galeria/" . $id . "/thumb/" . $nome_imagem;
